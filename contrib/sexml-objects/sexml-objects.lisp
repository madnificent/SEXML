  ...
  (let ((result "(")
	...
	(progn
	  ...
	  (let ((c (read-char stream nil #\null)))
	    (cond ((or (char= c #\/) (char= (peek-char t stream) #\/))
		   (loop until (char= (read-char stream) #\>))
		   (return-from read-tag (concatenate 'string result ")")))
	  ...
	  (loop for c = (read-char stream nil #\null)
	     do
	       (if (and reading-string-p (or (char= c #\<) (char= c #\/) (char= c #\>)))
		   (progn
		     (setf reading-string-p nil)
		     (setf result (concatenate 'string result "\" "))))
	       (cond ((and (char= c #\<) (char= (peek-char nil stream) #\#))
		      (setf result (concatenate 'string result " " (read-lisp-form stream) " ")))
		     ((and (not reading-string-p)
			   (char= c #\space) (not (char= (peek-char t stream) #\/)) (not (char= (peek-char t stream) #\>)) (not (char= (peek-char t stream) #\<)))
		     ...
		     ((char= c #\null)
		      (return-from read-tag result))
		     (t
		      (cond ((and (not reading-string-p) (not (find (peek-char t stream) '(#\> #\< #\/))))
			     (setf reading-string-p t)
			     (setf result (concatenate 'string result " \"" (string c))))
			    (t (setf result (concatenate 'string result (string c))))))))))))
